...
```shell
curl "https://api.geocod.io/v1.7/geocode?q=1109+N+Highland+St%2C+Arlington+VA&fields=cd118&api_key=YOUR_API_KEY"
curl "https://api.geocod.io/v1.7/reverse?q=38.886672,-77.094735&fields=cd118&api_key=YOUR_API_KEY"
```
...
<aside>
<strong>Note:</strong> When using our spreadsheet upload tool instead of the API, "Full FIPS (block)" is equivalent to `full_fips` in our API response. Similarly, "Full FIPS (track)" is equivalent to `full_fips` with the last 4 characters removed. 
<aside>

...
function template($country, $fields, $includeResponse) {
	...
	```shell
	curl "https://api.geocod.io/v1.11/geocode?q=$addressUrlEncoded&fields=$commaSeparated&api_key=YOUR_API_KEY"
	curl "https://api.geocod.io/v1.11/reverse?q=$coordinate&fields=$commaSeparated&api_key=YOUR_API_KEY"
	```
...
function getApiResponse(string $address, array $fields) {
	...
	if (getenv('DISABLE_API')) {
		...
		$url = "https://api.geocod.io/v1.11/geocode?q=". urlencode($address) ."&fields=". implode(',', $fields) ."&api_key=$apiKey";
